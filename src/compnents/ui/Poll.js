...
class Poll extends React.Component {
    ...
    feed.then(res => {
      if (res.length > 0) {
        ...
        this.setState({
          data: res,
          current: res[random],
          randomArr: [...this.state.randomArr, random]
        });
      } else {
        this.setState({ fetchingErr: true, message: "Error In Fetching Data" });
      }
    });
  }
  ...
  handleMouseOver(target) {
    ...
    const a = target.closest(".choices").querySelector(".a");
    const b = target.closest(".choices").querySelector(".b");
    const d = target.closest(".choices").querySelector(".d");
    arr.map(item => {
      if (target.classList.contains("a") && item.classList.contains("a")) {
        ...
        b.classList.add("loses");
        d.classList.add("loses");
      } else if (
        target.classList.contains("b") &&
        ...
        item.classList.add("wins");
        a.classList.add("loses");
        d.classList.add("loses");
      } else if (
        target.classList.contains("d") &&
        item.classList.contains("d")
      ) {
        while (item.classList.length > 0) {
          item.classList.remove(item.classList.item(0));
        }
  ...
  render() {
    if (this.state.fetchingErr) {
      return (
        <div>
          <ShowMessage msg={this.state.message} onClick={this.handleClick} />
    ...
    const currentIsVoted = this.state.voted.find(
      obj => obj.vote.id === this.state.current.id
    );
    ...
    return (
      <section
        className={`main-container ${sport && sport.toLowerCase()}`}
      ...
      >
        ...
        <button
          className="btn-submit Enter"
          type="submit"
          onClick={this.handleSubmit}
          tabIndex="4"
        >
          Vote...{" "}
          <span className="random">{randomMsg[Services.random(0, 4)]}</span>
        </button>
        <ShowMessage msg={this.state.message} onClick={this.handleClick} />
      </section>
    );
